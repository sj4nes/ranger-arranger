...
pub fn normalize_components<T: RangeSubtypeOps>(
...
fn mr_contains_element_inner<T: RangeSubtypeOps>(
    ...
    Ok(components
        .iter()
        .any(|c| crate::engine::contains_range(c, &range)))
}

fn mr_union_inner<T: RangeSubtypeOps>(a_buf: &[u8], b_buf: &[u8]) -> Result<Vec<u8>, String> {
...
pub fn mr_merge_buffers<T: RangeSubtypeOps>(a_buf: &[u8], b_buf: &[u8]) -> Result<Vec<u8>, String> {

...
mod slice4 {
    ...
    use vsql_ranger_arranger::func::range_agg::{
        datemr_range_agg_accumulate, datemr_range_agg_clear, datemr_range_agg_result,
    ...
    fn int8mr_range_agg_merges_components() {
        ...
        int8mr_range_agg_accumulate(
            &mut state,
            &[InValue::Custom(int8mr_encode("{[1,3)}").unwrap().as_slice())],
        );
        int8mr_range_agg_accumulate(
            &mut state,
            &[InValue::Custom(int8mr_encode("{[7,10)}").unwrap().as_slice())],
        );
        ...
        let comps = mr_decode_to_vec::<vsql_ranger_arranger::subtype::int8::Int8Ops>(&bytes).unwrap();
    ...
    fn int4mr_range_agg_merges_components() {
        ...
        int4mr_range_agg_accumulate(
            &mut state,
            &[InValue::Custom(int4mr_encode("{[1,3)}").unwrap().as_slice())],
        );
        int4mr_range_agg_accumulate(
            &mut state,
            &[InValue::Custom(int4mr_encode("{[7,10)}").unwrap().as_slice())],
        );
        ...
        let comps = mr_decode_to_vec::<vsql_ranger_arranger::subtype::int4::Int4Ops>(&bytes).unwrap();
        assert_eq!(comps.len(), 2);
        assert_eq!(comps[0].lower, 1);
        assert_eq!(comps[0].upper, 3);
        assert_eq!(comps[1].lower, 7);
    ...
    fn datemr_range_agg_merges_components() {
        ...
        datemr_range_agg_accumulate(
            &mut state,
            &[InValue::Custom(datemr_encode("{[2026-01-01,2026-01-03)}").unwrap().as_slice())],
        );
        datemr_range_agg_accumulate(
            &mut state,
            &[InValue::Custom(datemr_encode("{[2026-01-07,2026-01-10)}").unwrap().as_slice())],
        );
        ...
        let comps = mr_decode_to_vec::<vsql_ranger_arranger::subtype::date::DateOps>(&bytes).unwrap();
        assert_eq!(comps.len(), 2);
    ...
    fn dtmr_range_agg_merges_components() {
        ...
        dtmr_range_agg_accumulate(
            &mut state,
            &[InValue::Custom(dtmr_encode("{[2026-01-01 00:00:00,2026-01-03 00:00:00)}").unwrap().as_slice())],
        );
        dtmr_range_agg_accumulate(
            &mut state,
            &[InValue::Custom(dtmr_encode("{[2026-01-07 00:00:00,2026-01-10 00:00:00)}").unwrap().as_slice())],
        );
        ...
        let comps = mr_decode_to_vec::<vsql_ranger_arranger::subtype::datetime::DateTimeOps>(&bytes).unwrap();
        assert_eq!(comps.len(), 2);
